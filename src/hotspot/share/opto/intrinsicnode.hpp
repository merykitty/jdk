...
private:
  ...
  virtual const TypePtr* out_adr_type_impl() const { return TypeAryPtr::BYTES; }
...
private:
  ...
  virtual uint hash() const { return StrIntrinsicNode::hash() + (uint)(uintptr_t) _out_adr_type + (uint)(uintptr_t) _in_adr_type; }
  virtual bool cmp(const Node& n) const {
    const StrInflatedCopyNode& s = static_cast<const StrInflatedCopyNode&>(n);
    return StrIntrinsicNode::cmp(n) && _out_adr_type == s._out_adr_type && _in_adr_type == s._in_adr_type;
  }
