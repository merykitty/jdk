...
bool LibraryCallKit::inline_encodeISOArray(bool ascii) {
  ...
  const TypeAryPtr* out_adr_type = TypeAryPtr::BYTES;
  Node* mem = capture_memory(in_adr_type, TypeAryPtr::get_array_body_type(src_elem), out_adr_type);
  Node* enc = new EncodeISOArrayNode(control(), mem, in_adr_type, src_start, dst_start, length, ascii);
  enc = _gvn.transform(enc);
  Node* res_mem = _gvn.transform(new SCMemProjNode(enc));
  set_memory(res_mem, out_adr_type);
  set_result(enc);
  clear_upper_avx();
